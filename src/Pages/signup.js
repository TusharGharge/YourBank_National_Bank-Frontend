...
function Signup() {
  ...
  const [count, setCount] = useState(0);
  const [error, setError] = useState(null);
  const [datatushar, setDatatushar] = useState(false);
  ...
  const handlesubmit = (event) => {
    ...
    axios.post('https://' + process.env.REACT_APP_SERVER_URL + '/user/', data)
      .then(res => {
        console.log(res);
        setCount(0);
        navigate('/signin');
      ...
      .catch(error => {
        ...
        setCount(2);
        setError('Credentials are already used');
      })
      .finally(() => {
        setDatatushar(false);
        event.target.reset();
          ...
          <div className="md:flex p-10">
            ...
            <form onSubmit={handlesubmit}>
              <div className='main'>
                ...
                <div className='control'>
                  <label htmlFor='confirm-password'> Confirm password</label>
                  <input id="confirm-password" type="password" name="confirm-password" required maxLength={15} />
                </div>
                <div className="control-error">{count === 1 && <p className='text-red-600 font-moto'>{error}</p>}</div>
                <div className='control'>
                 <label htmlFor="phoneno">Phone number</label>
  <input id="phoneno" type="tel" name="phone_number" required maxLength={10} minLength={10} />
               </div>
                <p>{count === 2 && <p className='text-red-500 font-moto'>{error}</p>}</p>
                <p className="form-actions">
                  <button type="reset" className="button button-flat">
